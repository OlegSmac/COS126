...
public class KernelFilter {
    private static Color calculation(Picture picture, int col, int row, double[][] kernel, double k) {
        double sumred = 0;
        double sumgreen = 0;
        double sumblue = 0;
        for (int i = 0; i <= kernel.length - 1; i++) {
            for (int j = 0; j <= kernel[i].length - 1; j++) {
    ...
    public static Picture identity(Picture picture) {
        ...
        for (int col = 0; col < picture.width(); col++) {
            for (int row = 0; row < picture.height(); row++) {
                res.set(col, row, calculation(picture, col, row, k, 1));
            }
        }
        return res;
    }
    ...
    public static Picture gaussian(Picture picture) {
        ...
        for (int col = 0; col < picture.width(); col++) {
            for (int row = 0; row < picture.height(); row++) {
                res.set(col, row, calculation(picture, col, row, k, 1/16.0));
            }
        }
        return res;
    }
    ...
    public static Picture motionBlur(Picture picture) {
        ...
        for (int col = 0; col < picture.width(); col++) {
            for (int row = 0; row < picture.height(); row++) {
                res.set(col, row, calculation(picture, col, row, k, 1/9.0));
            }
        }
        return res;
    }
